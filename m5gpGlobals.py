...
# ***************************************************************
OP_IF =  -10017     # Conditional operator (if)
OP_IFE = -10017    # Conditional special operator equal to (if ==)
OP_IFG = -10018    # Conditional special operator greater than (if >)
OP_IFL = -10019    # Conditional special operator less than (if <)

# ***************************************************************
# Si se agrega un nuevo operador, incrementar el valor de OP_END
# ***************************************************************
OP_END = -10020 # Final operator
...
OPERADORES_MASTER = {
    ...
    "Add": OP_SUM, # Summation operator
    "Mul": OP_PRD, # Product operator
    ...
    "if<": OP_IFL # Conditional special operator less than (if <)
}
...
# ##########################################################################################
# Maximum allowed limit for any semantic value generated by aggregations.
# Some datasets can have large values, which is why 1e8 is reasonable and less restrictive.
# This is used to prevent overflow and NaN values in the GPU computations, 
# which can cause the entire evaluation to fail.
# ##########################################################################################
SAFE_AGG_MAX = 1.0e8

# ##########################################################################################
# Value used when there are no valid elements. 
# For example, when all elements are NaN or Inf, or when there are no elements at all.
# ##########################################################################################
SAFE_AGG_FALLBACK = 0.0

# ##########################################################################################
# Epsilon for avoiding log(0), numerically unstable divisions, etc.
# This is a small value that prevents division by zero and log of zero, 
# which can cause NaN or Inf values.
# ##########################################################################################
SAFE_EPS = 1.0e-12
...
# This is used to determine if a value is close enough to zero to be treated as zero,
# which can help prevent numerical instability in operations like division and logarithms.
# ##########################################################################################
SAFE_IF_EPS = 1.0e-6       # Recommended tolerance for float32
...
def cudaSetup(gpu_device_number=0): 
    ...
    if torch.cuda.is_available():
        ...
        # limits = gpCuda.get_cuda_grid_limits()
...
def getIndividualExpr(config,  
    ...
    for i in range(config.GenesIndividuals):
        ...
        if (gene == OP_ADD) :
            ...
            Expr = Expr + "if\t"       
        #if ((gene <= -1000) and (gene > -10000)) :
        elif ((gene <= VAR_INI) and ((gene >= maxVar)) and (is_integer_like(gene))) :
            numVars = numVars + 1
            Expr = Expr + "X"
...
def getGeneExp(config, gene):
    ...
    # Si el gen es una variable dentro del rango permitido, devolver su nombre formateado
    if (
        gene <= VAR_INI
        and gene >= maxVar
        and is_integer_like(gene)
    ):
        index_var = int((float(gene) + var_ini) * (-1))
        return "X_" + str(index_var)

    # Si el gen es una constante dentro del rango permitido, devolver su valor formateado
    if gene >= MIN_CONSTANT and gene <= MAX_CONSTANT:
        return format_number(gene)

    # Si el gen es un operador conocido, devolver su símbolo
    if gene in OPERADOR_POR_ID:
        return OPERADOR_POR_ID[gene]
...
def getStackModelExpr(config, Model) :
    ...
    for i in range(lenModel):
        ...
        # ********************************* Es una variable ************************************/
        elif ((gene <= VAR_INI) and ((gene >= maxVar)) and (is_integer_like(gene))) :  # Es una variable
            tmpExpr= "1:"
            tmpExpr += geneExpr
            stackModel.put(tmpExpr)
            lenIndiv += 1
